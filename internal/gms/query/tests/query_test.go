...
// the method is a safe no-op. This documents which chain methods the
// Interceptor actually calls for each public method under test.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Interceptor — constructor and Priority
// ---------------------------------------------------------------------------
...
func TestParsedQuery_CallsQueryReceived(t *testing.T) {
	...
	if err := qi.ParsedQuery(&stubChain{}, &vitessmysql.Conn{}, "SELECT 1", nil, noopQueryCallback); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !called {
...
func TestParsedQuery_QueryReceived_Error_ShortCircuits(t *testing.T) {
	...
	err := qi.ParsedQuery(chain, &vitessmysql.Conn{}, "SELECT 1", nil, noopQueryCallback)
	if !errors.Is(err, want) {
		t.Errorf("expected %v, got %v", want, err)
	}
	if chainCalled {
